...
class AG:
    ...
    def coste_mutacion(self, cromosoma_actual, coste_actual, i, j):
        ...
        coste_mutacion = coste_actual - sum(
            np.sum(self.mP[[i, j], :] * self.mD[cromosoma_actual[[i, j], None], cromosoma_actual], 1))
        ...
        coste_mutacion -= sum(
            sum(self.mP[idx][:, [i, j]] * self.mD[cromosoma_actual[idx, None], cromosoma_actual[[i, j]]]))
    ...
    def AG(self, parametros):
        ...
        dimension = self.dim
        print("Se está ejecutando el algoritmo genético Balwin en un problema de dimensión: ", self.dim)
        comienzo = time.time()
        ...
        opt = Opt(dimension, self.mD, self.mP)
        ...
        scoresOpt = opt.twoOptBalwin(parent[0])  # apply 2opt to the best individuals
        parent[0]["coste"] = scoresOpt
        ...
        for i in range(2):
            ...
            baseName = "dataGAbalwin/PC" + str(probabilidad_cruce) + "PM" + str(probabilidad_mutacion)
            ...
            pares_seleccionados = zip(participantes_idx[0:2 * numero_cruces:2],
                                      participantes_idx[1:2 * numero_cruces:2])
            ...
            for par, indice_1, indice_2 in zip(pares_seleccionados, range(0, 2 * numero_cruces, 2),
                                               range(1, 2 * numero_cruces, 2)):
                hijo[indice_1], hijo[indice_2] = self.cruce(parent[par[0]], parent[par[1]], puntos_cruce[indice_1],
                                                            puntos_cruce[indice_2])
            ...
            costesOpt = opt.twoOptBalwin(parent[0])
            parent[0]["coste"] = costesOpt

            parent.sort(order="coste", kind='mergesort')

            print("Coste mejor padre en la generación (AGB)", i, int(parent[0]["coste"]))
