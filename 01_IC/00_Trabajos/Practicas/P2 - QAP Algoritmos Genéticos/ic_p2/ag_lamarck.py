...
class AG:
    ...
    def AG(self, parametros):
        ...
        dimension = self.dim
        print("Se está ejecutando el algoritmo genético Lamarck en un problema de dimensión: ", self.dim)
        comienzo = time.time()
        ...
        for i in range(10):
            ...
            pool = multiprocessing.Pool(processes=8)
            parent[:nOpts] = pool.map(opt.twoOpt, parent[:nOpts])
            pool.close()

            parent.sort(order = "coste", kind = 'mergesort')
            print("Coste mejor padre en la generación (AGL)", i, int(parent[0]["coste"]))
