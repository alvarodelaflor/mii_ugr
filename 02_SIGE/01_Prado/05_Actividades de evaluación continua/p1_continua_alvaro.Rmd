...
auc1 <- roc(val$Survived, predictionValidationProb[["Yes"]], levels = unique(val[["Survived"]]))
...
```{r}
rpartCtrl2 <- trainControl(verboseIter = TRUE, classProbs = TRUE, summaryFunction = twoClassSummary, method = "cv", number = 20)
...
auc2 <- roc(val$Survived, predictionValidationProb2[["Yes"]], levels = unique(val[["Survived"]]))
...
cm_train1 <- confusionMatrix(predict1, val[["Survived"]])
cm_train2 <- confusionMatrix(predict2, val[["Survived"]])
cm_train1
cm_train2
```