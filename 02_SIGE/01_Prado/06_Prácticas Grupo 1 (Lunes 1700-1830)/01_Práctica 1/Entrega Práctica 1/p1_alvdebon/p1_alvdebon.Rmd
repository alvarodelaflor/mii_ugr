...
```{r densidad}
ggplot(training_data_higgs) +
  geom_density(aes(x = PRI_jet_num, fill = Label, color = Label), alpha = 0.3) +
  labs(x = "PRI_jet_num", y = "") +
  scale_fill_discrete(name ="Clase", labels=c("(b)ackground", "higg(s)")) +
...
En esta sección en primer lugar trataremos los valores perdidos, para ello hemos decido utilizr el valor 39% como máximo (siguiendo las recomendaciones de alguno de los foros que hemos visitado).

También aprovecharemos para eliminar aquellas variables cuyos valores presentan más del 80% de valores distintos, ya que realmente no aportan información útil para nuestro sistema.
