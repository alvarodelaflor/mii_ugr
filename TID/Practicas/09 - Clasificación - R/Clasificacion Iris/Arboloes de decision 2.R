#Ejemplo de arboles de decison con tree
...
entrena=iris[ind==1,]
test=iris[ind==2,]
#Seleccionar la libreria tree, se obtiebe un arbol de decision basado en entropia
library("tree")
...
iris_arbol2=tree(modelo,data=entrena)
table(predict(iris_arbol2,type="class"),entrena$Species)
summary(iris_arbol2)
plot(iris_arbol2) ; text(iris_arbol2)
#expandir el arbol para verlo completo e interpretarlo
#Ver que pasa con los test
testpred=predict(iris_arbol2,newdata=test,type="class")
