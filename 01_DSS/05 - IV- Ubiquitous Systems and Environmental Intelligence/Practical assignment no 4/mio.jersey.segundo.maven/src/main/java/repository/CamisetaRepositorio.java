...
public class CamisetaRepositorio {
	...
	public static Camiseta anadirCamisetaNavegador(String id, String modelo, Double precio, String talla, Boolean vendida, String image, String imageShop, String location, String envio) {
		Camiseta res = null;
		try {
			factoria = Persistence.createEntityManagerFactory(PERSISTENCE_UNIT_NAME);
			EntityManager em = factoria.createEntityManager();
			Camiseta camiseta = new Camiseta(id, modelo, talla, precio, vendida, image, imageShop, location, envio);
	...
	public static List<Camiseta> obtenerCamisetaPorUsuario(String id) {
		factoria = Persistence.createEntityManagerFactory(PERSISTENCE_UNIT_NAME);
		EntityManager em = factoria.createEntityManager();
		@SuppressWarnings("unchecked")
		List<Camiseta> carrito = em.createQuery("SELECT x FROM Carrito ca JOIN ca.camisetas x WHERE ca.id = '" + id + "$carrito'").getResultList();
		return carrito;
	}
}