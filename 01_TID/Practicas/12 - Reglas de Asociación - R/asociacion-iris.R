...
#Capturar el fichero asociacion.iris.csv que se ha generado con Knime y que ya est? prpeprado con datos categ?ricos
#Lo convertimos en una BD de transacciones, vemos informacion acerca de el
iristrans=as(asociacion.iris,"transactions")
summary(iristrans)
...
sale1=apriori(asociacion.iris, parameter = list(support = 0.1, confidence = 0.8, minlen = 2))
#salida del conjunto de reglas
summary(sale1)
sale3=sort(sale1,by="confidence")
salida1=inspect(sale1,ruleSep = "---->", itemSep = " + ", setStart = "", setEnd ="", linebreak=TRUE)
#restriccion de reglas, solo las que predicen clases y con confianza mayor que 0.9
sale2=subset(sale3,subset=rhs %in% c("class=Iris-versicolor","class=Iris-virginica","class=Iris-setosa")& confidence >= 0.9)
